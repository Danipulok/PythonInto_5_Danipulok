...
# Также записать в новый файл всех учащихся в формате "Фамилия И. Ср. балл":
...
file_inp = open('14.2.input.txt', 'r', encoding='utf-8')
inp = file_inp.read()
file_inp.close
...
print('Ученики со среднем баллом ниже пяти: ')
aver = []  # List of average marks
aver_ = 0  # Float of average for whole class
for i in range(len(lst)):
    sum_ = 0
    for j in range(2,13):
        sum_ += int(lst[i][j])
    aver.append(round((sum_ / 12), 2))
    aver_ += sum_/12
    if (sum_ / 12) < 5:
        print('•', lst[i][:2][0], lst[i][:2][1])
print('Средний балл по классу —', round((aver_/12), 2))

file_output = open('14.2.output.txt', 'w', encoding='utf-8')
for i in range(12):
    # file_output.write(lst[i][:2][1], lst[i][:2][0][0] +  + ' '*(7 - len(str(aver[i]))))
    file_output.write(lst[i][:2][1])  # Second name
    file_output.write(' ')  
    file_output.write(lst[i][:2][0][0])  # First name
    file_output.write('.')
    file_output.write(' '*(16 - len(lst[i][:2][1])))
    file_output.write(str(aver[i]))  # Average
    file_output.write('\n')
file_output.close
